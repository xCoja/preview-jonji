...
const MANUAL_PRIZES = [500, 250, 125, 75, 50, 0, 0, 0, 0, 0];
...
const CASINOS = {
  thrill: {
    ...
    endsInDays: 8,
    players: [], // We'll keep this empty until you have an API for Thrill
  ...
  clash: {
    ...
    endsInDays: 8,
    players: [], // We'll keep this empty until you have an API for Clash
    prevPeriod: 'Last Reset — June 2026',
    previousWinners: [],
  },
  juice: {
    ...
    letter: 'J',
    logo: 'juice.png',
    pool: 1000, // Updated to match your $1000 prize pool
    desc: "is a CS2 skins casino. Play cases and games with code",
    endsInDays: 14,
    players: [], // This will be populated dynamically by the fetch function
    prevPeriod: 'Last Reset — June 2026',
    previousWinners: [
...
async function fetchJuiceData() {
  try {
    ...
    // Sort by wagered amount descending and slice to max 10
    ...
    // Inject into CASINOS object
    CASINOS.juice.players = leaderboard;
...
function render() {
  ...
  countdownTarget = new Date(Date.now() + c.endsInDays * 86400000);
...
/* ---- INITIALIZE ---- */
// Fetch the live data, then render everything
fetchJuiceData();
...
if (loginBtn) {
  loginBtn.addEventListener('click', async () => {
    try {
      // Calls your existing Netlify function to get the Kick OAuth URL
      const res = await fetch('/.netlify/functions/kick-login');
      const data = await res.json();
      if (data.url) {
        window.location.href = data.url; // Redirect to Kick
      }
    } catch (err) {
...
async function checkAuth() {
  try {
    // Calls your existing Netlify function to check session
    const res = await fetch('/.netlify/functions/get-user');
    if (res.ok) {
      const user = await res.json();
      
      // If user exists, swap the UI
      if (user && user.username) {
        if (loginBtn) loginBtn.style.display = 'none';
        if (userMenu) userMenu.style.display = 'flex';
        if (navUsername) navUsername.textContent = user.username;
        
        // Only set avatar if it exists, otherwise leave the blank fallback
...
if (logoutBtn) {
  logoutBtn.addEventListener('click', () => {
    // Clear cookies based on your old script logic
    document.cookie = 'kick_session=; Path=/; Max-Age=0';
    document.cookie = 'kick_session=; Path=/; Domain=.jonji.bet; Max-Age=0';
    
    // Call your Netlify logout function if you have one, or just refresh
